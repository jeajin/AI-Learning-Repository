...
class SACController():
    def __init__(self, n_action=len(env.action_space.high)):
        ...
        self.memory = RBuffer(1_00_000, (8,), len(env.action_space.high)) #env.observation_space.shape
    ...
    def update_target(self, tau=None):
        ...
        for i, weight in enumerate(self.value_net.weights):
            ...
            # print(np.array(weights1[0][0]).shape,'i:',i)
            # print(np.array(weight * tau + targets1[i] * (1 - tau)).shape)
        self.target_value_net.set_weights(weights1)

    def train(self):
        if self.memory.cnt < self.batch_size:
            print(self.memory.cnt < self.batch_size)
            return
        ...
        with tf.GradientTape() as tape1, tf.GradientTape() as tape2, tf.GradientTape() as tape3, tf.GradientTape() as tape4:
            ...
            print("states", states.shape)
            print("v_actions", v_actions.shape)
...
with tf.device('GPU:0'):
    ...
    agent = SACController(2)
    episodes = 20000 # start나 내부 변수로 조정
    ep_reward = [] # episode당 점수
    total_avgr = [] # 해당 episode의 점수 기록
    target = False # 점수 넘었는지
    with tf.device('GPU:0'):
