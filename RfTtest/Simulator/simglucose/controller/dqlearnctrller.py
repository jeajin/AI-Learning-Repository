...
class DQLController(Controller):
    def __init__(self, state_size, action_size, episode, previous_time, model):
        ...
        self.BGmemory = deque(maxlen=self.previous_time)
    ...
    def policy(self, state):
        ...
        if np.random.rand() <= self.epsilon:
            ...
            q_value = self.model.predict(np.reshape(list(self.BGmemory), (1, 48, 1)))
    ...
    def BG_append_sample(self, state):
        self.BGmemory.append(state)

    def append_sample(self, state, action, reward, next_state, done):
        self.BG_append_sample(state)
        self.memory.append((list(self.BGmemory), action, reward, next_state, done))
    ...
    def train_model(self):
        ...
        # 현재 상태에 대한 모델의 큐함수
        target = self.model.predict(np.reshape(states, (self.batch_size, 48, 1)))
        # 다음 상태에 대한 타깃 모델의 큐함수
        target_val = self.target_model.predict(np.reshape(next_states, (self.batch_size, 48, 1)))
        ...
        self.model.fit(np.reshape(states, (self.batch_size, 48, 1)), target, batch_size=self.batch_size,
                       epochs=1, verbose=0)
